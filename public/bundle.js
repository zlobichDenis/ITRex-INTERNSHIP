...
var AuthorizationFormModel = /*#__PURE__*/function () {
  function AuthorizationFormModel() {
...
var RestorePassword = /*#__PURE__*/function (_AbstractComponent) {
  ...
  _createClass(RestorePassword, [{
    ...
    key: "getTemplate",
    value: function getTemplate() {
      return createRestorePasswordMarkup(this.email);
    }
  }, {
    ...
    key: "_isShowingAlertMessage",
    value: function _isShowingAlertMessage(element, isValid, fieldName) {
      if (!isValid) {
        element.querySelector(".feedback-message_".concat(fieldName)).classList.add("feedback-message_show");
        return;
      }

      element.querySelector(".feedback-message_".concat(fieldName)).classList.remove('feedback-message_show');
    }
  }, {
    key: "_inputsValidation",
    value: function _inputsValidation(element) {
      var _this3 = this;

      element.querySelector('#feedback-field__input_email').addEventListener('input', function (evt) {
        _this3.email = evt.target.value;
        _this3._isValidEmail = validator__WEBPACK_IMPORTED_MODULE_2___default().isEmail(evt.target.value) || validator__WEBPACK_IMPORTED_MODULE_2___default().isEmpty(evt.target.value);

        _this3._isShowingAlertMessage(element, _this3._isValidEmail, 'email');
      });
    }
  }]);
...
var SignIn = /*#__PURE__*/function (_AbstractComponent) {
  ...
  _createClass(SignIn, [{
    ...
    key: "rerender",
    value: function rerender() {
      _get(_getPrototypeOf(SignIn.prototype), "rerender", this).call(this);
    }
  }, {
    key: "_subscribeOnEvents",
    value: function _subscribeOnEvents() {
    ...
    value: function _inputsValidation(element) {
      ...
      element.querySelector('#feedback-field__input_pass').addEventListener('input', function (evt) {
        _this3.password = evt.target.value;
        _this3._isValidPass = !validator__WEBPACK_IMPORTED_MODULE_2___default().isAlpha(evt.target.value) || validator__WEBPACK_IMPORTED_MODULE_2___default().isEmpty(evt.target.value);

        _this3._isShowingAlertMessage(element, _this3._isValidPass, 'password');
      });
    }
  }, {
    key: "_showHidePasswordToggle",
    value: function _showHidePasswordToggle(element) {
      element.querySelector('.feedback-field__icon_password').addEventListener('click', function () {
        var passInput = element.querySelector('.feedback-field__input_password');
...
var SignUp = /*#__PURE__*/function (_AbstractComponent) {
  ...
  _createClass(SignUp, [{
    key: "getTemplate",
    value: function getTemplate() {
      return createSignUpMarkup(this.email, this.password);
    }
  }, {
    key: "recoveryListeners",
    value: function recoveryListeners() {
    ...
    value: function _setChangeActiveAuthStateHandler(element) {
      ...
      element.querySelector('.feedback-text__link').addEventListener('click', function () {
        return _this2._onChangeActiveAuthStateHandler(_common_const__WEBPACK_IMPORTED_MODULE_1__.AuthState.SIGN_IN);
      });
    }
  }, {
    key: "_isShowingAlertMessage",
    ...
    key: "_initEvents",
    value: function _initEvents(element) {
      var _this4 = this;

      element.querySelector('.feedback-field__icon_password').addEventListener('click', function (evt) {
        _this4._showHidePasswordToggle(evt.currentTarget);
      });
      element.querySelector('.feedback-field__icon_password-confirm').addEventListener('click', function (evt) {
        _this4._showHidePasswordToggle(evt.currentTarget);
      });
    }
  }, {
    key: "_showHidePasswordToggle",
    value: function _showHidePasswordToggle(target) {
      var parentOfTarget = target.parentElement;
      var passInput = parentOfTarget.querySelector('.feedback-field__input_password');

      if (passInput.getAttribute('type') === 'password') {
        parentOfTarget.querySelector('.feedback-field__icon').classList.add('feedback-field__icon_show');
        passInput.setAttribute('type', 'text');
        return;
      }

      parentOfTarget.querySelector('.feedback-field__icon').classList.remove('feedback-field__icon_show');
      passInput.setAttribute('type', 'password');
    }
  }]);
