...
const createSignUpMarkup = (emailValue = '', passwordValue = '') => {
    return (
        `<div class="feedback-wrapper feedback-wrapper_sign-up">
...
export default class SignUp extends AbstractComponent {
    ...
    _setChangeActiveAuthStateHandler(element) {
        element.querySelector('.feedback-text__link').addEventListener('click', () =>  this._onChangeActiveAuthStateHandler(AuthState.SIGN_IN));
    }
    ...
    _initEvents(element) {
        element.querySelector('.feedback-field__icon_password').addEventListener('click', (evt) => {
            this._showHidePasswordToggle(evt.currentTarget);
        });
        element.querySelector('.feedback-field__icon_password-confirm').addEventListener('click', (evt) => {
            this._showHidePasswordToggle(evt.currentTarget);
        });
    }

    _showHidePasswordToggle(target) {
        const parentOfTarget = target.parentElement;
        const passInput = parentOfTarget.querySelector('.feedback-field__input_password');
